...
Secure access will be provided by HTTPS Basic Access Authentication: each request between client and server will be authenticated with user/thirdparty credential.
...
\begin{itemize}
	...
	\item \textit{\textbf{/api/user/request/\{request\_id\}}}
		...
		\end{itemize}
	\item \textit{\textbf{/api/thirdparty/registration}}
		\begin{itemize}
			\item POST: register a new thirdparty with credentials specified in http fields.
		\end{itemize}
...
Another important design decision the team has taken is not to operate directly with wearable devices but instead to rely on mobile OS's feature to acquire data. In particular: the system is going to take advantage of Apple Healtkit to read data. This'll let the application to communicate with all wearable devices that are already compliant with Apple's framework without having to be compliant with each vendor's specific device.
\end{document}